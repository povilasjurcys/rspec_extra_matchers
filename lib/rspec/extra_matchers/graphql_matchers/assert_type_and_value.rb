...
module RSpec
  module ExtraMatchers
    module GraphqlMatchers
      ...
      class AssertTypeAndValue
        def initialize(type:, value_parent:, field_name: nil, checked_records: Set.new, deeply:, strictly:, property: nil)
        ...
        def assert_type_and_value
          ...
          return assert_nested_fields(value, type: type) if assert_deeply?(value)
        end
        ...
        def add_missing_method_error
          add_error(:missing_field, property: property, record: value_parent.inspect)
        end
        ...
        def assert_basic_type
          ...
          add_error(:wrong_type, expected_type: expected_type, actual_type: value.class.to_s)
        end
        ...
        def assert_nested_type(value_parent, type:, suffix: '', property: self.property)
          full_field_name = "#{field_name}#{suffix}"
          binding.pry if full_field_name == 'locations[0]'
        ...
        def add_error(type, **message_options)
          @detailed_error_messages << { type: type, field_name: field_name, **message_options }
        end
